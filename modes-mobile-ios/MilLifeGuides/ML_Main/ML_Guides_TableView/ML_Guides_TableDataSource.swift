 ...
 */
   ...
   func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        parentVc?.vc3?.fromMilLifeGuides = true
        parentVc?.vc3?.selectedGuide = parentVc?.viewModel?.selectedGuide
        parentVc?.vc3?.loadGuide()
    
    FirebaseUtility.shared.ContentCardSelected(screenName: "MilLifeGuides", contentType: "MILLIFE GUIDES", contentTitle: (parentVc?.viewModel!.selectedGuide)!, category: (parentVc?.viewModel!.selectedCategory)!, displayFormat: "view by category")
    }
