...
class MilLifeGuidesController: UIViewController {
    ...
    @IBAction func backfromGuidesTouched(_ sender: Any) {
        print("backfromGuidesTouched on MilLifeGuidesController")
        showMainView(view: GuidesTableView)
    }
    @IBAction func btn_backfromDetailsTouched(_ sender: Any) {
        print("btn_backfromDetailsTouched on MilLifeGuidesController")
    ...
    @objc func goHome(){
        ...
        //Clear Back Button Flags on Guides and Benefits Details
    ...
    override func viewDidLoad() {
        ...
        if self.vc3?.fromHomeVC == true || self.vc3?.fromFavVC == true {
            DetailsGuide.isHidden = false
        } else {
            DetailsGuideCont.isHidden = true
            showByCat()
        }
    ...
    @objc func handleGesture(gesture: UISwipeGestureRecognizer) -> Void {
        if gesture.direction == UISwipeGestureRecognizer.Direction.right {
            print("Swipe Right")
            if DetailsGuideCont.isHidden == false {
                vc3!.didTouchDetailsBackBtn(vc3!)
                return
            }
            if GuidesTableView.isHidden == false {
                print("call TableView Back Button")
                backfromGuidesTouched(self)
            }
        }
    }
