 ...
 */
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if((imageName) != nil){
            ...
            {
                let imageURL = URL(fileURLWithPath: dirPath).appendingPathComponent("small-images/"+imageName! + "-200x200.jpg")
   ...
   func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    FirebaseUtility.shared.guideContentSelected(screenName: (parentVc?.guide?.Guide)!, guideContentType: "articles", guideContentTitle: (parentVc?.guide?.listArticles?[indexPath.row]?.name)!)
