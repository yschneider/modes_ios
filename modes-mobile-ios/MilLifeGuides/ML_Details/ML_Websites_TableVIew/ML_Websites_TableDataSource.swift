   ...
   func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    print("myIndex: ", myIndex!.count)
        if indexPath.row <= myIndex!.count - 1 {
            ...
            FirebaseUtility.shared.guideContentSelected(screenName: (parentVc?.guide?.Guide)!, guideContentType: "websites", guideContentTitle: (parentVc?.guide?.RelatedWebsitesURL?[indexPath.row])!)
