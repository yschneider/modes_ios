...
class BenefitsController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if self.vc3?.fromHomeVC == true || self.vc3?.fromGuidesVC == true || self.vc3?.fromFavVC == true {
            DetailsBenefits.isHidden = false
        } else {
            DetailsBenefits.isHidden = true
            showByCat()
        }
    ...
    @objc func handleGesture(gesture: UISwipeGestureRecognizer) -> Void {
        if gesture.direction == UISwipeGestureRecognizer.Direction.right {
            print("Swipe Right")
            if DetailsBenefits.isHidden == false {
                vc3!.backBtnTouched(vc3!)
                return
            }
            if BenefitsTableView.isHidden == false {
                print("call TableView Back Button")
                backfromBenefitsTouched(self)
            }
        }
    }
