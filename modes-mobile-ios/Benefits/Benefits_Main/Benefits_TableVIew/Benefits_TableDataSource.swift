 ...
 */
   ...
   func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        parentVc?.vc3?.fromBenefitsVC = true
        parentVc?.vc3?.loadBenefit()
        parentVc?.showOverlay(view: (parentVc?.DetailsBenefits)!)
    
    FirebaseUtility.shared.ContentCardSelected(screenName: "Benefits", contentType: "BENEFITS", contentTitle: (parentVc?.viewModel!.self.selectedBenefit)!, category: (parentVc?.viewModel!.selectedCategory)!, displayFormat: "view by category")
